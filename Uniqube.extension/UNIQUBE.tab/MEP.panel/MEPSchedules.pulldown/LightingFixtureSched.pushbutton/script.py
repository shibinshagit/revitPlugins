# -*- coding: utf-8 -*-
"""Per-panel lighting fixture schedules filtered by BIMSF_Container."""
...
def main():
    ...
    if not unique_ids:
        forms.alert(
            "No lighting fixtures with '{}' found.".format(PARAM_NAME),
            title="uniqube",
        )
        return

    with revit.Transaction("uniqube: Lighting Fixture Schedule"):
        created = 0
        for pid in unique_ids:
            sched_name = "{} Lighting Fixture".format(pid)
            ...
            new_sched = DB.ViewSchedule.CreateSchedule(
                doc, DB.ElementId(DB.BuiltInCategory.OST_LightingFixtures)
            )
    ...
    forms.alert(
        "{} lighting fixture schedule(s) created.".format(created),
        title="uniqube",
    )
